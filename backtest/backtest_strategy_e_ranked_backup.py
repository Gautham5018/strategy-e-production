...
def main():    p=argparse.ArgumentParser(description='Strategy E V7 5-minute backtest')
    ...
    for c in candidates:
        ts=c['signal_time'];day=ts.date()
        # Move independently-simulated trades that have already exited into the day's realized ledger.
        still_active=[]
        for x in active:
            if x['exit_time']<=ts:
                day_net[x['exit_time'].date()]+=x['net_pnl']; day_consecutive[x['exit_time'].date()]=day_consecutive[x['exit_time'].date()]+1 if x['net_pnl']<0 else 0
                closed_by_day[x['exit_time'].date()].append(x)
            else: still_active.append(x)
        active=still_active
        if day_count[day]>=a.max_entries:decisions.append({**c,'status':'REJECTED','reason':'MAX_ENTRIES_PER_DAY'});continue
        if len(active)>=a.max_open:decisions.append({**c,'status':'REJECTED','reason':'MAX_OPEN_POSITIONS'});continue
        if ts.time()>cut and day_count[day]==0:decisions.append({**c,'status':'REJECTED','reason':'FIRST_SIGNAL_CUTOFF'});continue
        if day_net[day]>=a.daily_profit_lock:decisions.append({**c,'status':'REJECTED','reason':'DAILY_PROFIT_LOCK'});continue
        if day_net[day]<=-a.daily_loss_limit:decisions.append({**c,'status':'REJECTED','reason':'DAILY_LOSS_LOCK'});continue
        if day_consecutive[day]>=a.max_consecutive_losses:decisions.append({**c,'status':'REJECTED','reason':'CONSECUTIVE_LOSS_LOCK'});continue
        bs=cache[c['bars_file']];signal_bar=bs[c['signal_idx']];entry_preview=bs[c['signal_idx']+1]['open'] if c['signal_idx']+1<len(bs) else signal_bar['close']
        snap=score_trade(symbol=c['symbol'],signal_time=ts,signal_open=signal_bar['open'],signal_high=signal_bar['high'],signal_low=signal_bar['low'],signal_close=signal_bar['close'],entry_price=entry_preview,bars=bs[:c['signal_idx']+1],market_bars=[m for m in market if m['ts']<=ts],max_risk_pct=a.max_risk_pct,min_adx=a.min_adx,min_relative_volume=a.min_relative_volume,min_atr_pct=a.min_atr_pct,max_atr_pct=a.max_atr_pct,score_threshold=a.score_threshold,start=start,end=end,allow_windows=tw,market_filter=a.market_filter)
        threshold=a.trade2_score_threshold if day_count[day]>=1 else a.score_threshold
        # second-trade open basket quality guard using latest common-time closes when available
        open_loss=0.0
        if day_count[day]>=1 and active:
            for x in active:
                xb=cache[x['bars_file']]; xi=find_bar(xb,ts)
                if xi is not None:open_loss+=(xb[xi]['close']-x['entry'])*x['qty']
        feature_pass=bool(snap.score>=threshold and snap.market_regime_ok and snap.time_window_ok and snap.risk_ok)
        reason='' if feature_pass else ('FEATURE_REJECT:'+'|'.join(snap.reasons or ('SCORE_BELOW_THRESHOLD',)))
        row={**c,'feature_score':snap.score,'feature_grade':snap.grade,'risk_pct':snap.risk_pct,'atr_pct':snap.atr_pct,'adx14':snap.adx14,'relative_volume':snap.relative_volume,'vwap':snap.vwap,'market_regime_ok':snap.market_regime_ok,'time_window_ok':snap.time_window_ok,'feature_reasons':'|'.join(snap.reasons),'trade_threshold':threshold,'open_positions_pnl_at_signal':open_loss}
        if not feature_pass: decisions.append({**row,'status':'REJECTED','reason':reason});continue
        if day_count[day]>=1 and open_loss<=a.second_trade_open_loss_limit: decisions.append({**row,'status':'REJECTED','reason':'SECOND_TRADE_OPEN_LOSS_GUARD'});continue
        tr=one_trade(c,bs,a.capital,a.leverage,a.final_r,a.time_stop,a.stagnation_pct,a.trailing_stop,a.trailing_lock_r,a.trailing_distance_r,a.fixed_charge,a.slippage_pct,a.break_even,a.break_even_r,a.break_even_lock_r,a.risk_sizing,a.risk_per_trade_inr)
        if not tr:continue
        row.update(tr)
        if tr['status']!='TRADE': decisions.append(row);continue
        row['signal_time']=c['signal_time'];row['symbol']=c['symbol'];row['remaining_qty']=tr['qty'];decisions.append(row);active.append(row);day_count[day]+=1
        # Realized P&L is known only at exit. Conservative serial guard below updates after each simulated trade by exit order.
