...
def main():
    ...
    for ts, group_iter in groupby(candidates, key=lambda x:x['signal_time']):
        ...
        # Release trades that have already exited before this signal timestamp.
        still_active=[]
        for x in active:
            if x['exit_time']<=ts:
        ...
        available_slots=min(
            max(0,a.max_entries-day_count[day]),
            max(0,a.max_open-len(active))
        )
        ...
        # Current open basket P&L is identical for all signals at this timestamp.
        open_loss=0.0
        ...
        # Score EVERY signal at this timestamp before selecting a trade.
        for c in group:
            ...
            # Preserve the original Strategy E rule:
            # first trade uses score_threshold; subsequent trade uses trade2 threshold.
            threshold=(
                a.trade2_score_threshold
                if day_count[day]>=1 else a.score_threshold
            )

            feature_pass=bool(
                snap.score>=threshold
                and snap.market_regime_ok
                and snap.time_window_ok
                and snap.risk_ok
            )

            reason='' if feature_pass else (
                'FEATURE_REJECT:'+
                '|'.join(snap.reasons or ('SCORE_BELOW_THRESHOLD',))
            )
        ...
        # Strongest setup gets the available slot.
